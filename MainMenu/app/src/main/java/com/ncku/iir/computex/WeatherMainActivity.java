...
public class WeatherMainActivity extends RequestActivity {
...
//        requestServer = new RequestServer(WeatherMainActivity.this) ;
        ...
        Set<String> sunscreenText = new HashSet<>(Arrays.asList(new String[] {"今天太陽很大，可以擦個防曬抵擋紫外線", "小心外頭的太陽，你可以擦個防曬"}));
...
//                }catch(InterruptedException e){
    ...
    private void getAllFeatures(String weatherText) {
        ...
        try {
            ...
            setRealTimeFeature(mainBoard_id[0], str_location);
            setRealTimeFeature(mainBoard_id[1], str_temperature);
            setRealTimeFeature(mainBoard_id[2], str_condition);
            setMainBoardBackground(mainBoard_id[3], str_code);

            // remind board
            setRemindIcons(str_code);
            ...
            setFeatureBoards(featureInfoList);
            ...
            for (int i=0; i<Days.size(); i++) {
                ...
                setForecastBoard(dateText, cond, temp, i);
    ...
    private void setForecastBoard(String date_text, String cond, String temp, int i) {
        Log.d(TAG, "[setting forecastBoard " + i);
        int icon_id = getForecastIconId(cond);

        SingleScrollLinearLayout ll_forecast = findViewById(forecasts_id[i]);
    ...
    private void setRealTimeFeature(int i, String text) {
        TextView feature = findViewById(i);
        Log.d(TAG, ">> setting main Board " + text);

        feature.setText(text);
    }

    private void setMainBoardBackground(int i, String cond) {
        LinearLayout bg = findViewById(i);
    ...
    private void setRemindIcons(String cond) {
        for (int i=0; i<remindBoard_id.length; i++) {     //Umbrella Mask Sunscreen Jacket
            ImageView Icon = findViewById(remindBoard_id[i]);
    ...
    private void setFeatureBoards( String[] text_array) {
        for (int i =0;i<featureBoard_id.length;i++) {
            String text = text_array[i];
            SingleFeatureBoardLinearLayout featureBoard = findViewById(featureBoard_id[i]);
...
//            api.robot.setExpression(RobotFace.DOUBTING);
//        }
//        else api.robot.speak(response) ;
    }
